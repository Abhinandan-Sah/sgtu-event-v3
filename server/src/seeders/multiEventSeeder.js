      ...
      try {
        const result = await query(`
          INSERT INTO event_registrations (
            student_id, event_id, registration_status, payment_status,
            registered_at
          )
          VALUES ($1, $2, $3, $4, $5)
          RETURNING id
...
async function seedVolunteerAssignments(eventIds) {
  ...
  const volunteersResult = await query('SELECT id, email FROM volunteers WHERE is_approved_by_admin = true LIMIT 10');
